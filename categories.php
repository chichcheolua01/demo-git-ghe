        ...
        <div class="col-span-8 md:pl-[101px] lg:pl-[218.95px]">
            ...
            <div class="p-[36px]">
                <div class="relative">
                    ...
                    <div class="absolute inset-x-0 top-1/2 flex justify-center"><h1 class="sm:text-xl md:text-2xl lg:text-4xl duration-500 text-white font-bold">Quản lý Danh Mục</h1></div>
                </div>
            </div>
            <div class="p-[36px] flex justify-between items-center">
                ...
                <div class="flex justify-between items-center">
                    <div class="flex justify-between items-center space-x-[10px]">
                        ...
                        <input class="w-[200px]" type="text" placeholder="Search">
                    </div>
                    <div>
                        <a href="./view/add-new-category.php"><button class="rounded-xl p-[10px] bg-[#38A169] text-white">Add New Category</button></a> 
                    </div>
                </div>
            </div>
            <!-- Div này tí nữa xoá đi nhé -->
            
            <div class="p-[36px]">
                <table class="w-full">
                    <thead>
                        <tr class="text-white grid grid-cols-3">
                            <th class="bg-[#5B5B5B] px-[10px] py-[12px] border border-gray-800 justify-start flex">#</th>
                            <th class="bg-[#5B5B5B] px-[10px] py-[12px] border border-gray-800 justify-start flex">Category Name</th>
                    ...
                    <tbody>
                        <tr class="grid grid-cols-3">
                            <?php foreach($categories as $key => $value): ?>
                                ...
                                <td class="px-[12.5px] py-[22px]"> <?php echo $value["categoryName"]; ?></td>
                                <td class="px-[12.5px] py-[22px] flex justify-center">
                                    <div class="flex-col lg:flex-row flex space-y-3 lg:space-y-0 lg:space-x-3">
                                        <a href="./view/update-category.php?id=<?php echo $value["id"]?>"><button class="px-[20px] py-[10px] bg-[#1E74A4] text-white rounded-lg">Update</button></a>    
                                        <a href="./controller/delete-category.php?id=<?php echo $value["id"]?>"><button class="px-[20px] w-full py-[10px] bg-[#AC3131] text-white rounded-lg">Delete</button></a>    
                                    </div>
                                </td>
                            <?php endforeach ?>
                        </tr>
